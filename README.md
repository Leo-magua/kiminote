...
- 🎨 **富文本编辑器** - 基于 TipTap.js (ProseMirror) 的现代化编辑器
  - 支持图片拖拽上传和点击上传
  - 支持多种文件类型的附件上传（PDF、Word、Excel 等）
  ...
  - 引用、分隔线、高亮文本
  - 完整的排版格式化工具栏
- ✍️ **Markdown 支持** - 支持完整的 Markdown 语法，可在富文本、预览和 Markdown 源码三种模式间自由切换
- 🖼️ **图片上传** - 支持本地图片上传和拖拽上传（JPG/PNG/GIF/WebP/SVG，最大 10MB）
- 📎 **附件管理** - 支持上传 PDF、Word、Excel、PPT、TXT 等各类文档附件（最大 50MB）
- ↩️ **撤销重做** - 完整的编辑历史支持，可随时撤销和重做操作
...
- **富文本编辑器**: TipTap.js v2.2+ (基于 ProseMirror)
  - StarterKit 提供基础编辑功能
  - Image 扩展支持图片插入
  - Table/TableRow/TableCell/TableHeader 扩展支持表格
